...
StepsPerDayTotal <- aggregate(data1$steps ~ data1$date, as.Date(data1[, "date"]), FUN=sum)
names(StepsPerDayTotal) <- c('date', 'steps')
head(StepsPerDayTotal)
StepsPerDayTotal<-StepsPerDayTotal$steps
...
```{r}
    ...
    data1<- data[complete.cases(data),]
    IntervalAverages <- aggregate(data1$steps ~ data1$interval, list(data1$interval), FUN=mean)
    nrow(IntervalAverages)
    names(IntervalAverages) <- c('interval', 'steps')
    int<-IntervalAverages$interval
    IntervalAverages<-IntervalAverages$steps
    names(IntervalAverages)<-int
```
...
* splitting data into weekday and weekend data frames:
```{r}
    WeekSepList <- split(dataNaRm,dataNaRm$wkend)
```
...
aggregate(dataNaRm$steps ~ dataNaRm$interval + dataNaRm$wkend, list(dataNaRm$interval), FUN=mean)
